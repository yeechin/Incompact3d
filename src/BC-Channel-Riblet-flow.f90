...
module channel_riblet
  ...
  PUBLIC :: init_channel_riblet, boundary_conditions_channel_riblet, postprocess_channel_riblet, &
            visu_channel_riblet, visu_channel_riblet_init, momentum_forcing_channel_riblet, &
            geomcomplex_channel_riblet

contains
  !############################################################################
  subroutine init_channel_riblet (ux1,uy1,uz1,ep1,phi1)
...
!
    ...
    elseif (iin == 4) then ! SEM
       call sem_init_channel_riblet(ux1, uy1, uz1)
    endif
  ...
  subroutine boundary_conditions_channel_riblet (ux,uy,uz,phi,ep1)
    ...
    if (.not. cpg ) then ! if not constant pressure gradient
       if (idir_stream == 1) then
          ...
          call channel_riblet_cfr(uz,two/three)
       endif
    end if
  ...
  end subroutine boundary_conditions_channel_riblet
  !############################################################################
  !!
  !!  SUBROUTINE: channel_riblet_cfr
  !!      AUTHOR: Kay Schäfer
  !! DESCRIPTION: Inforces constant flow rate without need of data transposition
  !!
  !############################################################################
  subroutine channel_riblet_cfr (ux, constant)
  ...
  end subroutine channel_riblet_cfr
  !############################################################################
  !############################################################################
  subroutine postprocess_channel_riblet(ux1,uy1,uz1,pp3,phi1,ep1)
  ...
  end subroutine postprocess_channel_riblet
  subroutine visu_channel_riblet_init(visu_initialised)
  ...
  end subroutine visu_channel_riblet_init
  !############################################################################
  !!
  !!  SUBROUTINE: visu_channel_riblet
  !!      AUTHOR: FS
  !! DESCRIPTION: Performs channel riblet-specific visualization
  !!
  !############################################################################
  subroutine visu_channel_riblet(ux1, uy1, uz1, pp3, phi1, ep1, num)
  ...
  end subroutine visu_channel_riblet

  !############################################################################
  !############################################################################
  !!
  !!  SUBROUTINE: momentum_forcing_channel_riblet
  !!      AUTHOR: Paul Bartholomew
  !! DESCRIPTION: Applies rotation for t < spinup_time.
  !!
  !############################################################################
  subroutine momentum_forcing_channel_riblet(dux1, duy1, duz1, ux1, uy1, uz1)
    ...
    if (itime < spinup_time .and. iin <= 2) then
       if (nrank==0.and.(mod(itime, ilist) == 0 .or. itime == ifirst .or. itime == ilast)) &
          write(*,*) 'Rotating turbulent channel riblet at speed ',wrotation
...
!	pause
  ...
  !############################################################################
  !############################################################################
  subroutine sem_init_channel_riblet(ux1, uy1, uz1)
  ...
  end subroutine sem_init_channel_riblet
  !############################################################################
end module channel_riblet
